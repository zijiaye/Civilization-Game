...
public class TerrainTileFX extends StackPane {
    ...
    public void updateTileView() {
        ...
        TerrainTile terraintile = this.getTile();

        this.getChildren().remove(icon);
        if (terraintile.isEmpty()) {
            this.overlay.setFill(Color.TRANSPARENT);
        } else {
            overlay.setFill(terraintile.getOccupant().getColor());
            icon = new ImageView(terraintile.getOccupant().getImage());
            this.getChildren().add(icon);
        }
        if (this.equals(GameController.getLastClicked())) {
            overlay.setFill(Color.FIREBRICK);
        }
    }
}
